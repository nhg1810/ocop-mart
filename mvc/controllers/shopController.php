...
class shop extends Controller
{
    ...
    {
        if (
            $this->ShopModel->GetAllCateProd() == false || $this->ShopModel->GetSale() == false
    ...
    public function GetProdByIdCategory($idCategory ,$pageNumber)
    {
        if (
            $this->ShopModel->GetAllCateProd() == false || $this->ShopModel->GetSale() == false
            || $this->ShopModel->GetHotProduct() == false
            || $this->ShopModel->GetSpecialProShop() == false
            || $this->ShopModel->GetNewProduct() == false
            ||$this->ShopModel->GetSumProductByCate($idCategory)==false
        ) {
            echo "in ra trang lỗi";
        } else {
            if($this->ShopModel->GetAllProductByCategory($idCategory,$pageNumber)==false){
                $set_all_prod_by_cate = "Không tồn tại sản phẩm ở danh mục này !!!";
            }else{
                $rs_all_prod_by_cate = $this->ShopModel->GetAllProductByCategory($idCategory,$pageNumber);
                for ($set_all_prod_by_cate = array(); $row = $rs_all_prod_by_cate->fetch_assoc(); $set_all_prod_by_cate[] = $row);
            ...
            $rs_cate_prod  = $this->ShopModel->GetAllCateProd();
            $rs_sale_prod  = $this->ShopModel->GetSale();
            $rs_sum_prod_by_id = $this->ShopModel->GetSumProductByCate($idCategory);
            for ($set_sum_prod_by_cate = array(); $row = $rs_sum_prod_by_id->fetch_assoc(); $set_sum_prod_by_cate[] = $row);
            for ($set_special_prod = array(); $row = $rs_special_prod->fetch_assoc(); $set_special_prod[] = $row);
            for ($set_hot_prod = array(); $row = $rs_hot_prod->fetch_assoc(); $set_hot_prod[] = $row);
            for ($set_new_prod = array(); $row = $rs_new_prod->fetch_assoc(); $set_new_prod[] = $row);
            for ($set_sale_prod = array(); $row = $rs_sale_prod->fetch_assoc(); $set_sale_prod[] = $row);
            for ($set_cate_prod = array(); $row = $rs_cate_prod->fetch_assoc(); $set_cate_prod[] = $row);
            $this->view("mainView", [
                "control" => "prodbycategory",
                "id_cate_gory"=> $idCategory,
                "set_sum_prod_by_cate"=>$set_sum_prod_by_cate,
                "set_all_prod_by_cate"=>$set_all_prod_by_cate,
                "set_special_prod" => $set_special_prod,
                "set_hot_prod" => $set_hot_prod,
                "set_new_prod" => $set_new_prod,
                "set_sale_prod" => $set_sale_prod,
